...
```
思路：后序遍历，也是二叉树深度的运用，某节点的最大直径 = 左子树最大深度 + 右子树最大深度
```java
class Solution {
    int ans = 0;
...
思路：
- 前序遍历+前缀和哈希表，性能好
1. 利用前缀和：若 当前前缀和 - 目标值 = 某个历史前缀和，说明存在一些路径和为目标值
2. 哈希表记录每个前缀和出现的次数
3. 回溯保证只统计当前路径上的前缀和
```java
class Solution {
    ...
    public int pathSum(TreeNode root, int targetSum) {
        prefixSumCount.put(0L, 1);  // 初始化前缀和为0出现1次
    ...
    private void dfs(TreeNode node, long currentSum, int targetSum) {
        ...
        // 回溯：撤销当前节点的前缀和记录
        prefixSumCount.computeIfPresent(currentSum, (k, v) -> v - 1);
    }
}
```
- DFS，前序遍历
