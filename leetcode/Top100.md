...
```
## 49 字母异位词分组 middle
将每个字符串排序后作为key，利用HashMap的compute方法进行分组，相同排序结果的字符串归为一组
```java
class Solution {
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for (String str: strs) {
            ...
            Arrays.sort(arr);
            hash.compute(new String(arr), (k, v) -> v == null ? new ArrayList<>() : v).add(str);
        }
...
```
## 128 最长连续序列 middle
遍历哈希表即可，连续序列的起点即前一个数不存在于哈希表，当循环遍历到后一个数不存在于哈希表时，即到达了此连续序列的终点
```java
class Solution {
...
```
## 146 LRU缓存 middle
哈希表+双向链表的解法，用哈希表查找元素，用双向链表增删头尾元素。get若有则先将节点移到头部再返回其值，put若无该元素则size+1，增加节点到头部，且若超出限制还删掉尾部节点，若有该元素则更新值并移到头部
```java
//哈希表+双向链表
class LRUCache {
    class DLinkedNode {
...
# 双指针
## 283 移动零 easy
快慢指针，fast 指针遍历数组，遇到非零元素就赋值给 slow 指针位置，然后 slow 前移，遍历完成后，将 slow 指针之后的所有位置填充为 0
```java
class Solution {
...
```
## 11 盛最多水的容器 middle
首尾指针，移动短边才有可能会有更多的水，因为最低处决定水容量，需要尽可能高
```java
class Solution {
...
```
## 15 三数之和 middle
先排序，再固定一个元素，对其他元素首尾指针，时间复杂度O(N^2)
- 去重：跳过相同的固定元素、首元素、尾元素，避免重复结果 continue
- 优化：固定的元素（此时已是最小的元素）大于0时提前终止 break
```java
class Solution {
...
```
## 42 接雨水 hard
首尾指针，且需要两个值记录左、右边的最大高度，每次移动较矮一侧的指针，并累加该位置能接的雨水量。最大高度低的那边的指针可以决定能接多少水，接的量是最大高度-当前列的高度
```java
class Solution {
...
# 滑动窗口
## 3 无重复字符的最长子串 middle
用哈希来记录窗口内的元素用于判断是否出现过，每次for循环都是一个新的滑动窗口，每次新循环都要把哈希中新滑动窗口起始位置前一个字符删掉
```java
class Solution {
...
class Solution {
    public List<Integer> findAnagrams(String s, String p) {
        ...
        if (Arrays.equals(shash, phash)) res.add(0); //比较第1个窗口：若一样，说明已经找到了第一个异位词，索引是0
        for (int i = 0; i < sLen - pLen; i++) {
            shash[s.charAt(i) - 'a']--; //右移窗口，移除前一个窗口的首元素
            shash[s.charAt(i + pLen) - 'a']++; //右移窗口，加上前一个窗口的末元素后的元素。因为要找异位词，所以窗口长度必然是pLen
            if (Arrays.equals(shash, phash)) res.add(i + 1); //比较新的窗口，由于此时i代表前一个窗口的首元素，因此这里是i+1
        }
        return res;
    }
}
