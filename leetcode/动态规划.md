...
- 递推公式：text1第i-1下标元素和text2第j-1下标元素若相等则dp[i][j]=dp[i-1][j-1]+1，不相等则不考虑当前i-1下标元素（即text1[0,i-1)和text2[0,j)）、不考虑当前j-1下标元素（text1[0,i)和text2[0,j-1)），取较大值
...
- 递推公式
  - 若s[i-1]==t[i-1]，说明还存在子序列的可能，dp[i][j] = dp[i-1][j-1] + dp[i-1][j]
