...
```
思路：爬到第 n 阶的方法数 = 爬到第 n-1 阶的方法数 + 爬到第 n-2 阶的方法数，数学定义上正符合1 1 2 3...斐波那契数
- 动态规划，自底向上dp，迭代
```java
class Solution {
...
- 递推公式：dp[i][j] = dp[i - 1][j - 1] + dp[i - 1][j]，分别是当前元素左上方和上方的元素值。
...
class Solution {
    public List<List<Integer>> generate(int numRows) {
        ...
        for (Integer[] arr: dp) res.add(Arrays.asList(arr));
        return res;
    }
}
```
...
```
思路：走到当前单元格 (i,j) 的最小路径和 = “从左方单元格 (i,j-1) 与 从上方单元格 (i-1,j) 走来的 两个最小路径和中较小的 ” + 当前单元格值 grid[i][j]
- 左、上都是边界，即当i==0且j==0，此时是起点，dp[i][j] = grid[i][j]
- 左是边界，只能从上边过来，dp[i][j] = dp[i - 1][j] + grid[i][j]
- 上是边界，只能从左边过来，dp[i][j] = dp[i][j - 1] + grid[i][j]
...
- dp数组初始化：要初始化dp[0, j]和dp[i, 0]，由于到这个节点只能向右或向下，因此都只有1种走法，都初始化为1
- 遍历顺序：从左往右，从上到下
```java
class Solution {
...
- dp数组初始化：要初始化dp[0, j]和dp[i, 0]，由于只能向右或向下，且有障碍物，有障碍物的左边或上面是1，否则是0。且起始、终止有障碍直接返回0
- 遍历顺序：从左往右，从上到下
```java
class Solution {
...
```
思路：
- dp数组定义：dp[i]，对i拆分后得到的最大乘积，j表示拆分后的另一个值
- 递推公式：dp[i] = max(dp[i], j * (i - j), j * dp[i - j])
  - j * (i - j)将 i 拆分成两个数，再相乘
  - j * dp[i - j]是将 i 拆分成两个以上的数，拆分成 j 和 i-j 的拆分结果（i-j 继续拆分），再相乘
...
- 递推公式：以j为根节点，左节点有j-1个，右节点有i-j个，相乘可以得到j头节点的布局数，为了得到所有的，还应该累加，因此dp[i] += dp[j - 1] * dp[i - j]
...
先正序物品，再倒序背包
01背包，能不能装满容量j的背包，按价值=重量=元素值处理，dp[j] = Math.max(dp[j], dp[j - nums[i]] + nums[i])，最终dp[target]==target则说明装满重量target的背包的最大价值是target也即最大重量是target，说明此时已经装满
1. 计算数组总和，若为奇数则无法平分 
2. 使用 0-1 背包动态规划，寻找和为 sum/2 的子集 
3. dp[j] 表示容量 j 能装的最大值 
4. 先正序物品，再倒序背包容量避免重复使用同一元素 
5. 最终检查是否能凑出目标和
```java
class Solution {
...
- 递推公式：dp[j] += dp[j - nums[i]]，对于每个nums[i]
  - 不选它：方法数不变，仍是 dp[j]
  - 选它：方法数是 dp[j - nums[i]]（先装 nums[i]，再装剩余容量）
  - 总方法数 = 两者相加
- dp数组初始化：dp[0] = 1，说明把所有的元素都装到minus集合。j非0的初始化成0防止影响累加操作
- 遍历顺序：物品正序，容量倒序
```java
class Solution {
    public int findTargetSumWays(int[] nums, int target) {
        ...
        if (Math.abs(target) > sum || (target + sum) % 2 == 1) return 0;  //如果target的绝对值大于sum，或(target+sum)除以2的余数不为0，那么是没有方案的
...
```
思路：注意，这个题不是说每个元素都要符合m个0和n个1，而是整个子集内的所有元素共用这些0和1，有这个限制，因此要用到动态规划。每个元素只取1个或不取，因此是01背包，只是有两个维度：m个0和n个1，因此定义二维dp数组
- dp数组定义：dp[i][j] i个0 j个1的背包所能装的最多元素的个数
- 递推公式：dp[i][j] = max(dp[i][j], dp[i-zero][j-one] + 1)
  - dp[i][j]：不选当前物品（字符串）时，最多能组成的字符串个数（是上一轮循环结束后的状态）
  - dp[i-zero][j-one] + 1：选当前物品用了zero个0和one个1后，剩余容量能组成的最大数量，然后加上当前这一个字符串
- dp数组初始化：dp[0][0] = 0, 非0下标为防止覆盖max也初始化成0
- 遍历顺序：物品正序，容量倒序（先算出每个容量的两个维度分别是zero和one，这样才可以知道倒序遍历截止到哪个容量，也能算出如果要放这个物品时要先减掉多少weight）
```java
class Solution {
...
- dp数组初始化：dp[0] = 0，非零下标的元素初始化成amount+1防止影响min计算
- 遍历顺序：物品正序，背包正序，因为是组合，先遍历背包或物品都行
```java
class Solution {
...
- dp数组初始化：dp[0] = 1 什么都不装也是一种方法，不然后续都是0
- 遍历顺序：物品正序，背包正序。因为是组合，先遍历背包或物品都行
```java
class Solution {
...
- dp数组初始化：dp[0] = 0, 组成0不应该选任何物品，非零下标的元素初始化成整数最大值防止影响min计算
- 遍历顺序：正物正包。由于0并非可能的物品，因此从1开始遍历物品，然后是4 9...直到等于n
```java
class Solution {
...
```
思路：转化成完全背包问题，排列。背包容量即s的子串长度，一个物品是子串的每个分隔点，若能恰好装满则说明能拼出完整字串
* 题目特征：
  - 字典中的单词可以重复使用（无限个）→ 完全背包
  - 单词有顺序要求（"leet"和"code"组成"leetcode"）→ 排列问题
* 递推逻辑：
  - 分割点 i 将字符串分成 [0,i) 和 [i,j)，而i代表第i个元素前的位置，因此最多是s.length()
  - 若 [0,i) 能拆分(dp[i]=true) 且 [i,j) 是单词 → [0,j) 能拆
* 步骤：
  - dp数组定义：dp[j] 字符串s的j长度的子串是否能由wordDict组成
  - 递推公式：若dp[i] && wordDict.contains(s.substring(i, j))为true，则dp[j] = true
  - dp数组初始化：dp[0] = true，空串视为可拆分，不然后面的所有结果都是false
  - 遍历顺序：正包正物
```java
class Solution {
...
注意调用198方法时，由于1、2个元素的情况已经return，因此如果是3个元素，调用时分别是[0,2)、[1,3)，最少是2个元素，不用触发小于2个元素的异常情况
```java
class Solution {
    public int rob(int[] nums) {
        ...
        return Math.max(linear(Arrays.copyOfRange(nums, 0, nums.length - 1)), linear(Arrays.copyOfRange(nums, 1, nums.length)));
    }
    int linear(int[] nums) {
        int size = nums.length;
        int[] dp = new int[size];
...
```
思路：
- 贪心法。只收获每天的正利润。第1天买第3天卖，相当于1买2卖，2买3卖，因此只要知道每个区间即可算出任意某两天的买卖利润。而我们只收获正利润，当遇到负时由于可当天买卖即看做0。初始是0是因为把第一天看作当天买当天卖因此是0
```java
class Solution {
    public int maxProfit(int[] prices) {
        int result = 0;
...
```
- dp，相比于121问题，可以买卖多次。买卖多次和买卖一次，不持有股票的递推公式是一样的，而持有股票的不同，第i天买了股票造成了持有，因为可以买卖多次，买股票之前手上的钱不一定是0，而是dp[i-1][1]，因此这种情况下dp[i-1][1] - prices[i]
```java
class Solution {
    public int maxProfit(int[] prices) {
        int[][] dp = new int[prices.length][2];
...
- 递推公式
  ...
  - dp[i][1] = max(dp[i-1][1], dp[i-1][0] - prices[i]) 若是买入导致的第一次持有，则说明前一天是不操作的，即前一天不操作的金额-当天股票价格，也可以简写成-prices[i]；若是前一天买入导致的第一次持有，则是dp[i-1][1]
...
```
思路：与122问题类似，只是多了卖出后第二天不能买入的限制。而122问题的第i天的状态0持股、1不持股，不持股包含当天卖出、之前卖出，当天卖出会导致冷冻期，因此要拆分。
也就是说把之前122题的1状态拆成1、2、3，分别表示，状态：
- 0：持股 - 当天结束时持有股票
- 1：主动保持卖出 - 当天不持股，且当天没有卖出操作（之前已卖出，但主动保持卖出状态，非冷冻期导致）
- 2：当天卖出 - 当天卖出了股票
- 3：被动保持卖出 - 当天是冷冻期（前一天刚卖出，今天不能买入，被动保持卖出状态）
...
- 递推公式：
  - dp[i][0] = max(dp[i-1][0], dp[i-1][3] - prices[i], dp[i-1][1] - prices[i])
    ...
    - 买股，前一天是主动保持卖股状态 dp[i-1][1] - prices[i]
  - dp[i][1] = max(dp[i-1][1], dp[i-1][3]) 因为当前是主动保持卖股状态，因此前一天可能是同状态，也可能是冷冻状态
  - dp[i][2] = dp[i-1][0] + prices[i] 前一天只可能是保持买股状态，这样才可以当天卖出
  - dp[i][3] = dp[i-1][2] 前一天只可能是刚卖出的状态
...
- dp数组定义：dp[i] 以nums[i]结尾的最长子序列的长度，只是以nums[i]结尾，而不是全局的，因此要有个res记录全局最优结果
- 转移方程： 对于每个 i，遍历它之前的所有位置 j，得到本次迭代j后的最长的长度，注意不是全局最长的结果
  - dp[j]+1: 如果 nums[j] < nums[i]，说明可以把 nums[i] 接在以 nums[j] 结尾的递增子序列后面
  - dp[i]: 用来比较不同的j，nums[i]接在哪个j后面更长
...
```
思路：
- dp数组定义：dpMax[i]以第i个元素结尾的子数组乘积最大的值，dpMin[i]以第i个元素结尾的子数组乘积最小的值，如果前边累乘结果是一个负数，和当前负数累乘的话就会变成一个更大的数。所以我们还需要一个数组 dpMin 来记录以第 i 个元素的结尾的子数组作为乘积最小的值
- 递推公式，共三种乘积，dpMax算三者最大，dpMin算三者最小
  - dpMax[i - 1] * nums[i]
  - dpMin[i - 1] * nums[i]
  - nums[i] 一个新的子数组开始
...
class Solution {
    public int maxProduct(int[] nums) {
        int len = nums.length, res = nums[0];
        int[] dpMin = new int[len], dpMax = new int[len];
...
- 递推公式
  - 若s[i-1]==t[i-1]，dp[i][j] = dp[i-1][j-1] + dp[i-1][j] `TODO：还要多想想`
    - 用i-1元素匹配，因为两字串的最后一个字符已经相等了，不影响子序列个数，所以不需要考虑当前s子串和t子串的最后一位字母，dp[i-1][j-1]
    - 不用i-1元素来匹配，dp[i-1][j]，比如s="bagg", t="bag"，dp[4][3]也应包含s字串的前三个字符与t匹配的情况
  - 若s[i-1]!=t[i-1]，此时只能不用i-1元素来匹配，因此dp[i][j] = dp[i-1][j]
...
```
思路：最少删除次数 = (word1 需要删除的字符) + (word2 需要删除的字符)
= (len(word1) - len(LCS)) + (len(word2) - len(LCS))
= len(word1) + len(word2) - 2 * len(LCS)
...
- 动态规划
  ...
  - dp数组初始化：全初始化成false
  - 遍历顺序：因为需要依赖左下角的值，所以从下往上，从左往右，即i从大到小，j从小到大
```java
class Solution {
...
解释："aba" 同样是符合题意的答案。
```
思路：与647回文子串类似，只是647求的是回文子串个数，本题求最长的回文串，只要稍加增加判断条件用来记录更长的回文子串的起始索引即可
```java
class Solution {
    public String longestPalindrome(String s) {
        int len = s.length(), l = 0, r = 0, res = 0; // 记录左右边界和右-左的长度
        boolean[][] dp = new boolean[len][len];
        for (int i = len - 1; i >= 0; i--) {
            for (int j = i; j < len; j++) {
                if (s.charAt(i) == s.charAt(j) && (j - i <= 1 || dp[i + 1][j - 1])) {
        ...
        return s.substring(l, r + 1); //由于substring是左闭右开，因此要加1
    }
}
```
## 516 最长回文子序列 middle
...
- 递推公式：
  ...
  - 若s[i]!=s[j]，最长回文子序列只能从以下三种情况取较大值，而第三种情况不可能出现比前两种更长的回文子序列
    - 不包含左端点s[i]
    - 不包含右端点s[j]
    - 既不包含左端点又不包含右端点
