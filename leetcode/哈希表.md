...
函数 get 和 put 必须以 O(1) 的平均时间复杂度运行。
```
思路：两个哈希表和LinkedHashSet构建的双向链表
* min 是最小访问频次;
* HashMap<Integer, Node> cache 存缓存的内容; 
* HashMap<Integer, LinkedHashSet<Node>> freqMap 存每个访问频次对应的 Node 的双向链表（为了方便，直接用了 JDK 现有的 LinkedHashSet，其实现了 1 条双向链表贯穿哈希表中的所有 Entry，支持以插入的先后顺序对原本无序的 HashSet 进行迭代）
```java
class LFUCache {
