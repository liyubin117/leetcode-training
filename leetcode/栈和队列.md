...
```java
class MinStack {
    Stack<Integer> elementStack; //元素栈
    Stack<Integer> minStack; //对应元素的最小栈，与元素栈中的每个元素一一对应
...
```
思路：
通过栈维护可能形成有效括号的起始位置，动态计算最长有效长度。始终保持栈底元素为「当前最后一个没有被匹配的右括号的下标」，这样是为了用于判断边界条件，栈里的其他元素维护左括号的下标
- 遍历到每个 '('，将下标入栈
- 遍历到每个 ')'，先弹出栈顶元素用于匹配
  - 若栈为空，说明当前右括号没有被匹配，将其下标放入栈中来更新我们之前提到的「最后一个没有被匹配的右括号的下标」
  - 若栈非空，当前右括号的下标减去栈顶元素即为「以该右括号为结尾的最长有效括号的长度」

而且需要注意，如果一开始栈为空，第一个字符为左括号的时候我们会将其放入栈中，这样就不满足提及的「当前最后一个没有被匹配的右括号的下标」，为了保持统一，我们在一开始的时候往栈中放入一个值为 -1 的元素。
```java
class Solution {
...
使用栈结构遍历字符串，如果栈空或当前字符与栈顶不同则入栈，相同则出栈，最后将栈中剩余字符按顺序拼接成结果字符串
```java
class Solution {
...
class Solution {
    public int[] maxSlidingWindow(int[] nums, int k) {
        Queue<int[]> queue = new PriorityQueue<>((p1, p2) -> p2[0] == p1[0] ? p2[1] - p1[1] : p2[0] - p1[0]);
...
- 函数 addNum(num) ：
  - m!=n 此时N是奇数，m=n+1，需向small添加一个元素num，而由于num可能是在big即较大的一半范围内，因此先向big添加，然后把big堆顶元素弹出到small
  - m==n 此时N是偶数，m=n，需向big添加一个元素num
- 函数 findMedian()：
  - m!=n，中位数是big堆顶
...
class MedianFinder {
    ...
    public MedianFinder() {
        big = new PriorityQueue<>((p1, p2) -> p2 - p1);
