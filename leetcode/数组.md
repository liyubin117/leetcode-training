...
```
思路：最右节点一定是右段的最大值，且一定小于左段所有值，若中间节点大于其，则说明必在左段，反之则必在右段。
二分查找，考虑数组中的最后一个元素 x，在最小值右侧的元素（不包括最后一个元素本身），它们的值一定都严格小于 x；而在最小值左侧的元素，它们的值一定都严格大于 x。因此，我们可以根据这一条性质，通过二分查找的方法找出最小值
...
```
## 18 四数之和 middle
在三数之和基础上多了一层for循环，注意有一级剪枝/去重，二级剪枝/去重，之所以剪枝a时的判断条件是nums[k] >= 0 && nums[k] > target，比三数之和多了非负判断，是因为存在：(-2 -1 0 0) (-3)，若不加非负判断，就会跳过这种情况
```java
class Solution {
...
- 前缀和+哈希表，通过遍历数组，计算每个位置的前缀和，并使用一个哈希表来存储每个前缀和出现的次数。对于任意的两个下标i和j（i < j），如果prefixSum[j] - prefixSum[i] = k，那么说明从第i+1个位置到第j个位置的连续子数组的和为k，也就是若此前获取的前缀和中包含prefix[j]-k，那就说明以j为结尾的子数组有对应键值的次数符合条件
    - 使用哈希表记录“前缀和”对应的出现次数，初始化(0,1)是为了当某位置“前缀和”恰好等于k时不会漏掉
    - pre 累加当前元素得到前缀和
    - 若 pre - k 在哈希表中存在，说明有若干位置到当前位置的子数组和为 k
    - 更新哈希表中当前前缀和的计数
```java
//暴力枚举 O(N^2)
...
1. 第一次遍历：计算每个位置左侧所有元素的乘积存入 L 数组
2. 第二次遍历：从右向左，用变量 R 累乘右侧元素得到右侧乘积，L[i] = 左侧乘积 × 右侧乘积
```
原数组：           [1       2       3       4]
...
```
思路：对于某个位置 i，它能接的雨水量 = min(左侧最大高度，右侧最大高度)-当前高度。双指针，分别指向左右两端，且需要两个值记录左、右边的最大高度，最大高度低的那边的指针可以决定能接多少水，接的量是最大高度-当前列的高度。若左指针的leftMax比右指针的rightMax矮，说明：左指针的右边至少有一个板子 > 左指针左边所有板子。根据水桶效应，保证了左指针当前列的水量决定权在左边，那么可以计算左指针当前列的水量：左边最大高度-当前列高度
```java
class Solution {
...
class Solution {
    ...
    void mergeSort(int[] nums, int left, int right) {
        if (left >= right) return; // 终止条件，当数组长度为1时终止递归
    ...
    void merge(int[] nums, int left, int mid, int right) {
        ...
        // i, j分别是左子数组[left, mid]和右子数组[mid+1, right]的起始索引
